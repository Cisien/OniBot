...
if (Enter-OncePerDeployment "install_docker_image")
{
	...
	docker run -d -e "Token=$env:OniBotToken" --name OniBot -v onibot:c:\app\config --restart=always --isolation=hyperv "cisien/onibot:$tag"
	docker run -d -e "Token=$env:MeowBotToken" --name MeowBot -v meowbot:c:\app\config --restart=always --isolation=hyperv "cisien/onibot:$tag"
}