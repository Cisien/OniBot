...
namespace OniBot.Commands
{
    ...
    public class PermissionCommands : ModuleBase<SocketCommandContext>, IBotCommand
    {
        ...
        {
            await AddPermission(command, role.Id);
        }
        ...
        public async Task AddPermission(string command, ulong roleId)
        {
            ...
            var commands = await _handler.BuildHelp(Context);
            if (command != "*" && !commands.SelectMany(a => a.Commands).Any(a => a.Alias == command))
            {
                await ReplyAsync("Command either does not exist, or you do not already have permissions to it.");
                return;
            }
            ...
            await Configuration.Modify<PermissionsConfig>(_config.ConfigKey, async a =>
            {
                ...
                {
                    await ReplyAsync($"`{role.Name}` is already enabled for {command}");
                    return;
                }

                a.Permissions[command].Add(role.Id);

                await ReplyAsync($"{command} enabled for `{role.Name}`");
            }, Context.Guild.Id);
        }
        ...
        {
            await RemovePermission(command, role.Id);
        }
        ...
        public async Task RemovePermission(string command, ulong roleId)
        {
            ...
            await Configuration.Modify<PermissionsConfig>(_config.ConfigKey, async a =>
            {
                ...
                await ReplyAsync($"{command} disabled for `{role.Name}`");
            }, Context.Guild.Id);
        }
        ...
        public async Task ShowPermissions()
        {
            ...
            await Context.User.SendFileAsync(Encoding.UTF8.GetBytes(json), "permissions.json");
        }
